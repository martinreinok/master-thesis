...
"""
Augmentation 1s per image
  - slice thickness: weakening of contrast (maybe network specific)
  - distortions and noise: weakening of contrast (maybe network specific)
"""
...
"""
image_resolution = 200  # pixels (square ratio)
...
"""
Local Gradient Field Effect, from
"Passive Tracking and System Interfaces for Interventional MRI", Sunil Patil
"""
Gz = -3 * c * Z * ((-3 * X**2) - (3 * Y**2) + (2 * Z**2)) / ((X**2 + Y**2 + Z**2)**(7 / 2))
Gx = -3 * c * X * ((X**2) + (Y**2) - (4 * Z**2)) / ((X**2 + Y**2 + Z**2)**(7 / 2))
...
# SGz_coronal = 1 / slice_thickness * np.trapz(p_xyz * np.exp(-1j * gyromagnetic_ratio * Gz * TE), Y, axis=0)
# SGx_coronal = 1 / slice_thickness * np.trapz(p_xyz * np.exp(-1j * gyromagnetic_ratio * Gx * TE), Y, axis=0)
S_coronal = np.rot90(S_coronal)
# SGz_coronal = np.rot90(SGz_coronal)
# SGx_coronal = np.rot90(SGx_coronal)

phase = np.angle(S_coronal)
magnitude = np.abs(S_coronal)

# real and imaginary parts of the signal
S_coronal_real = np.real(S_coronal)
S_coronal_imag = np.imag(S_coronal)
# SGz_coronal_real = np.real(SGz_coronal)
# SGz_coronal_imag = np.imag(SGz_coronal)
# SGx_coronal_real = np.real(SGx_coronal)
# SGx_coronal_imag = np.imag(SGx_coronal)
...
Colormap
"""
bSSFP_colors = [(0, (0, 0, 0)), (0.5, (0.25, 0.25, 0.25)), (1, (1, 1, 1))]
cmap_name = 'custom_colormap'
n_bins = 1000
bSSFP_colors = LinearSegmentedColormap.from_list(cmap_name, bSSFP_colors, N=n_bins)
GRE_colors = "gray"

zoom = .5
# To match the image pixel resolution of the MRI image.
plt.figure(figsize=(image_resolution / 100, image_resolution / 100))
plt.imshow(magnitude, extent=(x.min(), x.max(), y.min(), y.max()), cmap=bSSFP_colors)
plt.axis('off')
plt.show()

plt.figure(figsize=(image_resolution / 100, image_resolution / 100))
plt.imshow(phase, extent=(x.min(), x.max(), y.min(), y.max()), cmap=bSSFP_colors)
plt.axis('off')
plt.show()

# plt.figure(figsize=(image_resolution / 100, image_resolution / 100))
# plt.imshow(SGz_coronal_real, extent=(x.min(), x.max(), y.min(), y.max()), cmap="gray")
# plt.axis('off')
# plt.show()
#
# plt.figure(figsize=(image_resolution / 100, image_resolution / 100))
# plt.imshow(SGx_coronal_real, extent=(x.min(), x.max(), y.min(), y.max()), cmap="gray")
# plt.axis('off')
# plt.show()
